...
## Phase 11: APIキー設定UI実装（予定工数: 3時間）

### 背景
現在のAI機能（Phase 7）は環境変数からのみAPIキーを取得する実装になっている。ユーザーがUI上でAPIキーを設定できる機能を追加し、ローカルストレージに保存する仕組みを実装する。
...
- XSS攻撃に対する脆弱性あり（Next.jsのデフォルト保護に依存）
- README.mdに「APIキーは個人利用のみ、共有端末では注意」を明記
...
### 完了条件
- [x] ホーム画面に「弱点診断」ボタンが表示され、診断が実行できる ✅
- [x] 結果画面の各問題カードに「もっと詳しく」ボタンが表示され、解説強化が実行できる ✅
- [x] クイズ画面に「AI問題を生成」ボタンが表示され、問題生成が実行できる ✅
- [x] APIキー未設定時は適切にボタンが非表示またはdisabled状態 ✅
- [x] 全ユニットテストがパス（152個全パス） ✅
- [x] 全E2Eテストがパス（既存18個 + Phase 13新規8個 = 26個実装） ✅
- [x] コードカバレッジ87%以上を維持（87.24%達成） ✅
- [x] ESLintエラー・警告ゼロ ✅
- [x] TypeScript型エラーゼロ ✅
...
### テスト品質
- [x] 単体テストカバレッジ80%以上 - ✅ **87.24%達成（ステートメント）**
  - ステートメント: 87.24%
  - ブランチ: 80.04%
  - 関数: 89.44%
  - ライン: 88.45%
- [x] 全テストパス - ✅ **152個のテスト全パス**
- [x] E2Eテスト実装完了 - ✅ **5つのE2Eテストファイル作成（26テストケース）**
...
- [x] Phase 13: AI機能のUI統合 - ✅ **完了（全5サブフェーズ完了、152テスト全パス、カバレッジ87.24%）**
